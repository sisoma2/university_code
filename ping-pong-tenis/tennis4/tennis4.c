...
void * moure_pilota(void * cap)
{
  ...
  do{
	...
	if ((f_h != ipil_pf) || (c_h != ipil_pc)){	/* si posicio hipotetica no coincideix amb la pos. actual */
		if (f_h != ipil_pf){
			waitS(id_sem);						/* provar rebot vertical */
			...
			if (rv != ' '){							/* si no hi ha res */
				...
				pal=(int)rv-48;
				if(pal>0 && pal<=num_paletes){
					if(pil_vc<0){
						vel=1;
						waitS(id_sem);
						sendM(id_bustia[pal-1],&vel,sizeof(int));
						signalS(id_sem);
					}
					else{
						vel=-1;
...
int main(int n_args, const char *ll_args[])
{
  ...
  id_mis=ini_mem(sizeof(int)*num_paletes); /* crear zona mem. compartida */
  id_bustia=map_mem(id_mis); /* obtenir adres. de mem. compartida */

  for(i=0;i<num_paletes;i++){
  	id_bustia[i]= ini_mis();
  }

  id_sem = ini_sem(1);  /* inicialitzem semafor */
  ...
  sprintf(a6,"%i",id_tec);
  sprintf(a7,"%i",id_pil);
  sprintf(a13,"%i",id_sem);
  sprintf(a14,"%i",id_mis);

   for(i=0;i<num_paletes;i++){
    tpid[i] = fork();
	if (tpid[i] == (pid_t) 0){  /* branca del fill */
		sprintf(a8,"%i",ipo_pf[i]);
		sprintf(a9,"%i",ipo_pc[i]);
		sprintf(a10,"%f",po_pf[i]);
		sprintf(a11,"%f",v_pal[i]);
		sprintf(a12,"%i",i);
